...
import { backendLogin, backendRegister } from '../services/mockBackend';
import { Lock, User as UserIcon, Loader2, Upload, AlertCircle, ShieldCheck, ArrowRight, CheckCircle2, Globe } from 'lucide-react';
...
export const Auth: React.FC<AuthProps> = ({ onLogin }) => {
  ...
  const [subStep, setSubStep] = useState<'FORM' | 'VERIFY_DOC'>('FORM');
            ...
            {mode === 'REGISTER' && (
              <div className="space-y-4">
                ...
                <button disabled={loading} className="w-full bg-white text-black font-bold py-3.5 rounded-xl hover:bg-gray-200 transition-colors flex items-center justify-center gap-2 mt-4 shadow-lg shadow-white/5">
                  {loading ? <Loader2 className="animate-spin w-5 h-5" /> : (regRole === UserRole.PROVIDER ? 'Continue to Verification' : 'Create Account')}
                </button>
              </div>
            )}
          </form>
